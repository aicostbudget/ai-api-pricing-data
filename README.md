...
The GitHub Pages `/api/v1` JSON and CSV publish the canonical V1 records in this repository (currently 39 models). The AICostBudget Website JSON/CSV and Hugging Face files publish the same 47-row Pricing V2 public Website projection, including structured tier and pricing-component metadata. Public export schema 1.5.0 includes `pricing_components` in JSON and `pricing_components_json` in CSV without changing existing compatibility fields. Hugging Face must match the Website projection exactly by key set and exported fields; it is not expected to match the intentionally narrower GitHub Pages V1 key set.
